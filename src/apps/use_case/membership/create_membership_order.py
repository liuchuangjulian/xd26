...
class CreateMembershipOrder:
    def __init__(self,
        ...
        self.wx_pay = wx_pay
    async def get_membership_obj(self, session, membership_id):
        # 获取可开通的会员信息
    ...
    async def last_order(self, session, user, membership):
        # 检查是否有未支付的订单
    ...
    async def execute(self, uid, membership_id) -> None:
        async with self.membership_order_repo.session as session:
            membership = await self.get_membership_obj(session, membership_id)
            user = await self.get_user_obj(session, uid)

            order = await self.last_order(session, user, membership)
            if not order:
                order = MembershipOrderEntity().create_order(uid, membership)
