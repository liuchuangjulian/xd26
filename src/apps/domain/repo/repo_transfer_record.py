    ...
    async def ini(self, session):
        create_biz_sql = """
                CREATE TABLE IF NOT EXISTS `transfer_record` (
                  ...
                  `real` int NOT NULL COMMENT '实际金额（分）',
                  `gift` int NOT NULL DEFAULT 0 COMMENT '赠送金额（分）',
                  `coin_count` int NOT NULL DEFAULT 0 COMMENT '积分',
                  `op_user_id` varchar(128) DEFAULT NULL COMMENT '操作用户id',
                  ...
                  PRIMARY KEY (`id`),
                  KEY `ix_transfer_record_user_id` (`user_id`),
                  KEY `ix_transfer_record_shop_id` (`shop_id`),
